...
const AUTH_KEY = "pt_auth_v1"; // { token, brand: { id, name, phone, logo } }
...
.hpill{font-family:'DM Sans',sans-serif;font-size:.58rem;letter-spacing:.2em;text-transform:uppercase;padding:.25rem .65rem;background:rgba(212,168,67,.12);border:1px solid rgba(212,168,67,.25);color:var(--gold);max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
export default function App() {
  ...
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    setAuthErr("");
    if (!authPhone.trim() || !authPass) {
      setAuthErr("Please fill in phone number and password");
      return;
    }
    if (authMode === "signup" && !authName.trim()) {
      setAuthErr("Please enter your brand name");
      return;
    }
    setAuthBusy(true);
    try {
      ...
      const nextAuth = { token: data.token, brand: data.brand };
      setAuth(nextAuth);
      persistAuth(nextAuth);
      AudioEngine.play("success"); haptic("success");
      setScreen("welcome");
    } catch (err) {
      setAuthErr(getFriendlyError(err.message));
      AudioEngine.play("error"); haptic("medium");
    } finally {
      setAuthBusy(false);
    }
  };
        ...
        {screen==="auth" && (
          <div className="auth">
            ...
            <div className="auth-sub">
              {authMode==="login" ? "Log in to your brand account" : "Create your brand account"}
            </div>

            <div className="auth-tabs">
              <button
                type="button"
                className={`auth-tab${authMode==="login"?" on":""}`}
                onClick={()=>{ click(); setAuthMode("login"); setAuthErr(""); }}
              >Log In</button>
              <button
                type="button"
                className={`auth-tab${authMode==="signup"?" on":""}`}
                onClick={()=>{ click(); setAuthMode("signup"); setAuthErr(""); }}
              >Sign Up</button>
            </div>
            ...
            <form className="auth-form" onSubmit={handleAuthSubmit}>
              {authMode==="signup" && (
                <div className="auth-field">
                  <label className="auth-label" htmlFor="auth-name">Brand Name</label>
                  <input
                    id="auth-name"
                    className="auth-input"
                    type="text"
                    placeholder="e.g. Pooja Textiles"
                    value={authName}
              ...
              {authErr && <div className="auth-err" role="alert">{authErr}</div>}

              <button type="submit" className="auth-submit" disabled={authBusy}>
                <span>{authBusy ? "Please wait…" : authMode==="login" ? "Log In" : "Create Account"}</span>
                {!authBusy && <span style={{fontSize:"1rem"}}>→</span>}
              </button>
            </form>

            <div className="auth-switch">
              {authMode==="login" ? (
                <>New brand? <button type="button" onClick={()=>{ click(); setAuthMode("signup"); setAuthErr(""); }}>Create an account</button></>
              ) : (
          ...
          <div className={`welcome${wExit?" out":""}`}>
            ...
            <div className="wcontent">
              ...
              <div className="wsub" style={{ marginTop: "1.5rem" }}>
                ...
                >
                  {(auth?.brand?.name || "Your Brand").toUpperCase()}
                </div>
