...
const publicBrand = (b) => ({ id: b.id, name: b.name, phone: b.phone, logo: b.logo || null });
...
router.post("/login", async (req, res) => {
  try {
    ...
    const brand = findByPhone(phone.trim());
...
export const requireAuth = (req, res, next) => {
  ...
  try {
    ...
    const brand = findById(payload.id);
...
router.put("/profile", requireAuth, (req, res) => {
  ...
  const updated = updateBrand(req.brand.id, updates);
